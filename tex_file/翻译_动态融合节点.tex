...
% Proof environment
\newenvironment{proofsk}{{\bf 证明（概要）:}}{\qed}

% === Title Information ===
\title{动态整数集合的最优排名、选择与前驱搜索\footnote{本文以不同格式发表在 {\em Proceedings of the 55nd IEEE Symposium on Foundations of Computer Science (FOCS)}, 2014, pp.~166--175.}}
\author{Mihai P\u{a}tra\c{s}cu\footnote{于2012年去世。} \and Mikkel Thorup\footnote{研究部分由丹麦独立研究委员会根据 Sapere Aude 研究职业计划的高级资助金支持。}\\[2ex]
哥本哈根大学}
\date{}
...
我们寻找 $w/b$ 个移位值 $s_i < k^2/2$，使得没有两个移位后的块 $\lpiece{B}{i}{k^2} \LSHIFT s_i$ 在同一位置有设置位。打包后的键将是 $2b$ 位的段：
\[
